...
def main():
    ...
    defconfigs = [f'{args.target}_defconfig', 'ssxv.config', f'{args.target}.config']
    if not args.no_ksu:
    ...
    zipname = 'Kernel-v{}_{}_{}_{}.zip'.format(
        kver, args.target, variantStr, datetime.today().strftime('%Y-%m-%d'))
    os.chdir('AnyKernel3/')
    zip_files(zipname, [
        'Image', 
